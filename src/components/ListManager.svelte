<script>
  ...
  async function loadMembersPage(page) {
    if (!$blueskyStore.session || !$listStore.selectedList || !$listStore.listMembers.length) return;
    ...
    try {
      ...
      console.log(`Loading page ${page}: DIDs ${startIndex + 1}-${Math.min(endIndex, $listStore.listMembers.length)}`);
...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div id="list-members-container" class="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col">
      <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-slate-800 mb-2">Latest List Members</h3>
        <p class="text-sm text-slate-600">
          Showing {isLoadingMembersPage ? '...' : $listStore.listMemberProfiles.length} of {$listStore.listMembers.length} total members
        </p>
      </div>
      ...
      <div class="p-6 flex flex-col flex-1">
        ...
        {:else if membersError}
          <div class="bg-red-50 border border-red-200 rounded-lg p-4 flex-1 flex items-center justify-center">
                  ...
                  >
                    <div class="flex items-center space-x-3">
                      <div class="flex-shrink-0">
                        <img
                          src={profile.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.displayName || profile.handle)}&background=6366f1&color=fff&size=64`}
                    ...
                    <div class="flex items-center space-x-1">
                      {#each Array.from({length: Math.min(5, getTotalPages())}, (_, i) => {
                        ...
                        <button
                          ...
                          class="px-3 py-1 text-sm border rounded transition-colors duration-200 {currentMembersPage === pageNum ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 hover:bg-gray-50'} disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {pageNum}
                        </button>
                      {/each}
